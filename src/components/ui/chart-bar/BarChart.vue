...
const colors = computed(() => props.colors?.length ? props.colors : defaultColors(props.categories?.length ?? 0))
const legendItems = ref<BulletLegendItemInterface[]>(Array.isArray(props.categories) ? props.categories.map((category, i) => ({
  name: category,
...
const xFormatter = computed(() => (v: number) => props.data?.[v]?.[index.value])
</script>
